...
class ArweavePurchaseService:
    ...
    WRAPPED_AR_CANDIDATES = [
        ...
        "A7n8jRSQn9YvRLr6V8ztkP2z2ypeEo7b6QksA1JForward",
        "JBoQUUqRpdB8oYbT8p7EQfR8nxZDj6z83ZV8b7Zz49sE",
    ]
    ...
    async def discover_arweave_token(self) -> Optional[str]:
        ...
        try:
            cached = self._token_cache.get("arweave_mint")
            if cached:
                return cached

            env_mint = os.getenv("ARWEAVE_OUTPUT_MINT", "").strip()
            candidate_mints = []
            if env_mint:
                candidate_mints.append(env_mint)
            ...
            self._set_last_error("No routable Arweave token mint found")
            return None

        except Exception as e:
            self._set_last_error(f"Error discovering Arweave token: {e}")
            return None
    ...
    async def get_quote(self, usdc_amount: float, output_mint: Optional[str] = None,
        ...
        try:
            ...
            if not target_mint:
                target_mint = await self.discover_arweave_token() or ""
    ...
    async def build_swap_transaction(self, user_pubkey: str, usdc_amount: float, 
        ...
        try:
            ...
            if not target_mint:
                target_mint = await self.discover_arweave_token() or ""
            if not target_mint:
                self._set_last_error("Unable to determine output mint for swap")
                return None
